    ...
    public partial class frmFilmCustomPropertyEdit : BaseEditForm<FilmCustomProperty>
    {
        ...
        protected override List<FieldForEditUI> FieldsNames()
        {
            return new List<FieldForEditUI>()
            {
                ...
                new FieldForEditUI(EditedObject, ReflectionHelper.Nameof<FilmCustomProperty>(s => s.CustomProperty), ReflectionHelper.Nameof<FilmCustomProperty>(s => s.CustomProperty.Value), "Значение", new Dictionary<Type, Type>() { { typeof(CustomProperty), typeof(frmCustomPropertyList) } })
            };
        }
